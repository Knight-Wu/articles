...
防止服务端接收到网络上滞留的请求连接包, 若没有第三次 ack 的过程, 服务端接收了此类包发送一个syn+ack, 并一直等待客户端发送数据, 导致浪费资源, 而且是死锁的情况, 若有第三次的ack, 当客户端收到syn+ack, 如果之前的包已经过时, 则客户端不会再次发送ack, 故服务器不会盲等.
...
主动关闭方第一次 FIN 表明没有数据传输了, 如果被动关闭方还有数据, 
就等待被动关闭方发送 FIN.
#### 为什么需要四次挥手

因为TCP 是全双工的协议, 双方都可以发送和接收, 必须要一方发送结束信号, 另一方ack 之后才表示结束, 否则经过前两次挥手之后只是单向断开, 另一方还可以发送数据, 
...
### TCP 为什么是可靠的传输, 提供了乱序重排、应答确认、报文重传和流量控制四种机制。
