# 数据库选型
其实针对后端常用业务, 就四种, RDBMS 关系型数据库 mysql , NOSQL 分为缓存 redis 和 宽列存储 hbase, Cassandra, 搜索引擎 es.</br> 文档型 MongoDB 和时序数据库, 图数据库都有专门特定的场景. </br> 数据库选型一般来说从读取的角度来考虑, 因为数据写入都是为了查的, 写入你怎么都可以写入, 问题是查能不能满足需求, qps , latency 等. 
## 关系型数据库
有事务, ACID, 支持 sql
