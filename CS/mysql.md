# 数据库选型
其实针对后端常用业务, 就四种, RDBMS 关系型数据库 mysql , KV redis, 宽列存储 hbase, Cassandra, 搜索引擎 es, 文档型和时序数据库, 图数据库都有专门特定的场景. 
## 关系型数据库
### 需求
有事务, ACID, 支持 sql

## KV 数据库
高吞吐量, 最终一致性, 对 sql 支持不友好或者不是原生支持, 可能不支持事务

## 宽列数据库
高吞吐量, 最终一致性, 对 sql 支持不友好或者不是原生支持, 可能不支持事务, 适合列式存储和搜索, 存大量数据, 持久化支持好. hbase, Cassandra
