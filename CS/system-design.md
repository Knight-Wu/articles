...
## 最终一致性分布式事务
TCC 事务是同步调用, 而基于MQ 的异步调用呢, 其实类似TCC , 只是把调用方式改成MQ 发消息, 然后还是需要有个中间件来接受各个服务的响应判断他们是否都try 成功了, 都成功了就confirm , 否则就cancel. 
## 对账两个订单表的方案
* 要求
  
设计一个对账方案  高效 且减少网络交互和带宽
AB两台机器分别存了
10TB的订单数据  (订单id  详情字段) 
...
1. 先按hash 分区, 订单id hash 之后分到多个partition, 每个partition 大小由测试决定, 太小太大都会影响性能, 跟我们采用的技术框架和机器都有关系, 分成多个分区之后就可以充分并发, MR 的思想
2. 再比较两个表同一个partition 的数据差异, 先排序, 按订单id, 然后对每行订单依次计算hash , hash(hash of order1 + hash of order2 + ...) 得到partition 的hash , 然后比对两个分区的hash
3. 如果hash 一致, 则两个分区一致, 如果不一致, 则把两个分区拉到内存中使用双指针算法逐行对比
4. 因为按照order id 排序了, 先比较 order[i], order[j], 如果相等, 则依次加一, 如果不等, 则orderid 小的加一, 并把不等的记录下来, 

## 设计一个注册中心
```
+-----------------------+
|     Client (SDK)      |  --> 服务注册、配置订阅、健康检查
+-----------------------+
           |
           ▼
+-----------------------+
|   Nacos Server Cluster|  --> 核心逻辑层（AP/CP 一致性协议）
|   +----------------+  |
|   | Naming Service |  |  --> 服务注册、发现、健康检查
|   +----------------+  |
|   | Config Service |  |  --> 配置管理、版本控制、灰度发布
|   +----------------+  |
|   |  Consistency   |  |  --> 数据一致性协议（Raft/Distro）
|   +----------------+  |
+-----------------------+
           |
           ▼
+-----------------------+
|      Storage Layer    |  --> 持久化存储（默认 Derby，可替换 MySQL、Redis 等）
+-----------------------+
```
模块: Naming Service（服务发现）、Config Service（配置管理） 和 Metadata Management（元数据管理）

Distro 协议：AP 模式下的自研协议，通过异步复制保证最终一致性。
Raft 协议：CP 模式下的强一致性协议，用于 Leader 选举和数据同步。

### 服务注册(写)
AP 模式（Distro 协议）：

当前节点（临时 Leader）接收请求，将实例信息写入内存（ConcurrentHashMap）。避免写入的时候还没来得及持久化就挂掉, 可以使用WAL. 内存可以设置LRU 策略, 并定时刷盘保证内存放得下. 

异步将注册信息同步给其他节点（最终一致性）。

CP 模式（Raft 协议）：

Leader 节点接收请求，通过 Raft 协议将数据写入日志并提交到多数节点。

数据持久化到存储层（如 MySQL）。

### 读
AP 模式：返回当前节点内存中的实例列表（可能包含未同步的最新数据）。但是如果返回旧的服务, 可以在客户端发送请求前探测一遍, 保证实例存活.

CP 模式：返回 Raft 协议确认的强一致性数据。

### 心跳
服务的节点主动保持心跳和leader 主动探测服务节点是否存活
