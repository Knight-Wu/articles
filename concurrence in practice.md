...
从1.8开始，采用CAS 机制，多个线程同时更新只有一个线程能成功。
    ...
    * put 操作
    val和key 都不能为null, 根据hash, hash & (tab.length-1), 算出数组的下标, 如果下标没有元素, 则进行一次CAS, 失败则进行下次循环, 如果该下标有元素, 则synchronize(头结点), 继续判断如果是链表, 则放在链表的末尾, 如果是红黑树, 则调用红黑树的插值方法, 如果链表长度超过了默认值
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEwNDgyNTQ1NywtMTgzNjEyMzgwNCw0ND
