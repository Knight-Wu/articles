...
* 之前的老办法，根据写入权重如何分配，又有什么问题。

一个es 节点的写入权重由这个节点的shard 的写入权重总和而成，某个shard 的权重由shard 的大小除以写入时间而得，比如 多少MB每秒， 是流量单位。 那么现在可得所有es 节点的写入权重了，按照升序排列，写入权重最小的节点就是第一个要选择的节点， 例如你要新增一个index， 此时没有监控数据，就只能默认给个shard 4，等到第一次滚动之后就有监控数据了，假设就是 n 个shard， 就遍历目前所有节点n 次， 每次取写入权重最小的节点，最后把这 n 个节点写入index 配置中。那么你如何感知集群的各个 节点的写入权重变化呢，
