...
#### POH 交易顺序的验证和eth 的不同和提升点
虽然都需要对新区块进行验证, 都是由proposer 定义交易的顺序并形成新区块, 但是eth 需要投票确认顺序是否合法, 通过重放整个区块的交易得到区块hash 对比hash, 有可能不合法, 但是solana poh 插入之后即
有了顺序, 但是仍然可能poh 验证不通过, 受到惩罚, 感觉还是没有本质的区别, 都是提议 + 验证 + 是否通过 ? 可以跟面试官讨论
![image](https://github.com/user-attachments/assets/cda67849-faa0-456c-95b8-ee7da18932f4)
...
每个 slot 是固定时间长度：12 秒一个 slot

所有节点使用本地系统时间，通过网络时间同步（比如 NTP）

每个节点只要系统时间到了 slot N 对应的时间，通过提前共享未来slot 的列表, 就知道:

当前是 slot N, 谁是 slot N 的 proposer

#### 时钟漂移
它有可能因为时钟漂移、网络延迟等原因，导致 slot 出块延迟或错过，所以：

Ethereum 的机制中允许：
proposer 迟点出块（你 slot 时间快结束了也能提）；

slot 没人出块也行（空 slot）；

validator 会对实际收到的区块进行投票，不盲信 proposer；

如果两个 proposer 同 slot 出块（罕见），会进入 fork，选择权重更高的一条。
