...
* 列压缩
适用于列中不同值的数量远小于行数,  将这一列的每一个不同值用位图编码, 每个不同的值分别用一个 bit 数组表示, 如果这个 bit 数组中零很多, 也可以用游程编码,  某个列值对应一个 bit 数组, 相同 index 下, bit 数组为 1, 
例如 id 列 1, 2 , 3, 9
name 列: a, b, c, d
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA4NDcyNzY5NiwxNjE1MDQzNTQ2LC0xNj
